...
extension String {
    public var isEmailValid: Bool {
        guard
            let range = self.range(of: regexInternationalEmail, options: [.regularExpression]),
            range.lowerBound == self.startIndex && range.upperBound == self.endIndex,
            // The numbers beneath here are as defined by
            // https://emailregex.com/email-validation-summary/
            self.count <= 320, // total length
            self.split(separator: "@")[0].count <= 64 // length before `@`
        else {
            return false
        }
